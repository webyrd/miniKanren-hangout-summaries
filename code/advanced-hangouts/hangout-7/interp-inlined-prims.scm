...
(define (eval-expo expr env val)
  (conde
    ...
    ((fresh (rator x* rands a* prim-id)
       (== `(,rator . ,rands) expr)
       (eval-expo rator env `(prim . equal?))
       (fresh (v1 v2)
         (== `(,v1 ,v2) a*)
         (conde
           ((== v1 v2) (== #t val))
    ...
    ((fresh (rator x* rands a* prim-id)
       (== `(,rator . ,rands) expr)
       (eval-expo rator env `(prim . symbol?))
       (fresh (v)
         (== `(,v) a*)
         (conde
           ((symbolo v) (== #t val))
